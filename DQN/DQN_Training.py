import numpy as np
import itertools
from DQN_model_new import Environment, Agent
from collections import deque
import copy
import time
import pickle

def train_dqn(episode, size, batch_size):
...
# Number of trailer types
t = [1, 3, 3]

#different batch sizes to test for 2x2 grid
b = [15, 32, 64]

#different batch sizes to test for 3x3 grid
# b = [32, 64, 128]
...
for day, trailer in zip(d, t):
    ...
    for batch in b:
        ...
        all_episode_losses, all_episode_rewards = train_dqn(1000,state_space_size,batch_size)  # give number of epoch as an argument
        end_time = time.time()
        print(end_time - start_time)
        with open(str(batch_size)+'DQN_'+str(state_space_size)+'_loss_results.pickle', 'wb') as fp:
            pickle.dump(all_episode_losses, fp)
        with open(str(batch_size)+'DQN_'+str(state_space_size)+'_rewards_results.pickle', 'wb') as fp:
